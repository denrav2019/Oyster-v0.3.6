...
O(1) доступ к обработчику по опкоду. Без switch/case.
...
4.3. Таблица дескрипторов функций (ТДФ)

typedef struct {
    uint16_t name;          // имя в пуле строк
    uint16_t module_name;   // имя модуля
    uint8_t param_count;      // количество параметров
    uint8_t local_count;      // количество локальных переменных
    uint32_t code_offset;     // смещение до кода функции
    uint8_t export_flag;      // флаг экспорта
} FunctionDef;
...
Сохраняет и восстанавливает контекст при вызове функций и модулей.

4.5. Загрузка модулей
VM загружает .ocm динамически при первом вызове функции модуля. Поиск модулей в @INC (по умолчанию: . и ./modules, плюс переменная окружения OYSTERLIB).

4.6. Файловые дескрипторы
FILE* open_files[256];
int open_file_count;
